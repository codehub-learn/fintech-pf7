    ...
    public void run(String... args) {
        ...
        if (accountService.findAll().isEmpty()) {
            ...
            accountService.addAll(
                    List.of(
                            Account.builder().name("Thomas Thomaidis").iban("GR44025635700006").type(AccountType.NORMAL).build(),
                            Account.builder().name("Dimitris Iraklis").iban("GR74813235701234").type(AccountType.NORMAL).build(),
                            Account.builder().name("Ioannis Danis").iban("GR42122635750096").type(AccountType.NORMAL).build(),
                            Account.builder().name("National Bank").iban("GR67225635712356").type(AccountType.BANKER).build()
                    )
            );
